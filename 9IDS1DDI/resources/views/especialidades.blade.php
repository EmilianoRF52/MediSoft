@extends('adminlte::page')
@section('title','Especialidades')
    
@section('content')

    <br>
            <div style=" background-color:#1466C3; color:#fff"  class="bg text-white p-2 mb-2 rounded-top">
                    <h3 class="text-center">Especialidades</h3>
                </div>
            </div>
            <div class="box-body">
                <table id="table-data" class="table table-bordered">
                    <thead style=" background-color:#1466C3; color:#fff">
                        <tr>
                            <th>Nombre</th>
                            <th style="width:22%; height:22%;" colspan="2">Opciones</th>
                        </tr>
                    </thead>
                    <tbody style=" background-color:rgb(255,255,255)" >
                        @foreach($especialidades as $especialidad)
                        <tr>
                            <td>{{$especialidad['nombre']}}</td>
                            <td>
                                <a href="{{route('nuevo.especialidad',['id' => $especialidad['id']])}}" class="btn btn-success btn-sm rounded-8">
                                    <span class=""><img src="data:image/png;base64,"/> Editar</span>
                                </a>
                            </td>
                            <td>
                                <a>
                                <form action="{{route('eliminar.especialidad',['id' => $especialidad['id']])}}" method="POST" >
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-sm rounded-8">
                                        <span class=""><img src="data:image/png;base64,"/> Eliminar</span>
                                    </button>
                                </form>
                                </a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
@endsection

@section('js')
<script>
    $('#table-data').DataTable({
         "scrollX": true
    });
</script>
 @stop