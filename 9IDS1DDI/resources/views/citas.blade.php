@extends('adminlte::page')
@section('title','Consultorios')
    
@section('content')

    <br>
            <div style=" background-color:#1466C3; color:#fff"  class="bg text-white p-2 mb-2 rounded-top">
                    <h3 class="text-center">Consultorios</h3>
                </div>
            </div>
            <div class="box-body">
                <table id="table-data" class="table table-bordered">
                    <thead style=" background-color:#1466C3; color:#fff">
                        <tr>
                            <th>Numero</th>
                            <th>Edificio</th>
                            <th>Nivel</th>
                            <th style="width:22%; height:22%;" colspan="2">Opciones</th>
                        </tr>
                    </thead>
                    <tbody style=" background-color:rgb(255,255,255)" >
                        @foreach($consultorios as $consultorio)
                        <tr>
                            <td>{{$consultorio['numero']}}</td>
                            <td>{{$consultorio['edificio']}}</td>
                            <td>{{$consultorio['nivel']}}</td>
                            <td>
                                <a href="{{route('nuevo.consultorio',['id' => $consultorio['id']])}}" class="btn btn-success btn-sm rounded-8">
                                    <span class=""><img src="data:image/png;base64,"/> Editar</span>
                                </a>
                            </td>
                            <td>
                                <a>
                                <form action="{{route('eliminar.consultorio',['id' => $consultorio['id']])}}" method="POST" >
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-sm rounded-8">
                                        <span class=""><img src="data:image/png;base64,"/> Eliminar</span>
                                    </button>
                                </form>
                                </a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
@endsection

@section('js')
<script>
    $('#table-data').DataTable({
         "scrollX": true
    });
</script>
 @stop